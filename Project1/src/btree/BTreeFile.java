...
/**
	...
	public BTreeFile(String filename, int keytype, int keysize, int delete_fashion) throws GetFileEntryException, ConstructPageException, IOException, AddFileEntryException 
	{
...
DeleteFileEntryException, ConstructPageException, PinPageException 
	...
	private void _destroyFile(PageId pageno) throws IOException, IteratorException, PinPageException,ConstructPageException, UnpinPageException, FreePageException 
	{
	...
	private void updateHeader(PageId newRoot) throws IOException, PinPageException, UnpinPageException 
	{
		...
		header.set_rootId(newRoot);				// clock in dirty bit to bm so our dtor needn't have to worry about it
...
/**
	...
	public void insert(KeyClass key, RID rid) throws KeyTooLongException, KeyNotMatchException, LeafInsertRecException,IndexInsertRecException, ConstructPageException, UnpinPageException, PinPageException,
...
// 1. headerPage.root == INVALID_PAGE:
		...
		if (headerPage.get_rootId().pid == INVALID_PAGE) 
		{
			...
			// ASSERTIONS:// - newRootPage, newRootPageId valid and 													     //pinned
			...
			if (trace != null) 
			{
				...
				return;
									...
									// ASSERTIONS:// - headerPageId, headerPage valid and pinned
   ...
   // - newRootEntry != null: a leaf split propagated up to the root
		...
		if (newRootEntry != null) 
		{
			...
			Object newEntryKey;				   // the information about the pair <key, PageId> is
...
DeleteRecException, IndexSearchException, UnpinPageException, LeafInsertRecException, ConvertException,
...

	{
		...
		if (currentPage.getType() == NodeType.INDEX)
		{
			...
			nextPageId = currentIndexPage.getPageNoByKey(key);	//now unpin the page, recurse and then pin it again
					...
					if (currentIndexPage.available_space() >=BT.getKeyDataLength(upEntry.key, NodeType.INDEX)) 
			{
				...
				unpinPage(currentIndexPageId, true /* DIRTY */);
					...
					return null;
								...
								// ASSERTIONS:   // - on the current index page is not enough space available .
													// it splits
...
+newIndexPageId +lineSep);
					...
					trace.writeBytes("ROOTSPLIT IN nodes " + currentIndexPageId + " " + newIndexPageId + lineSep);
							...
							// ASSERTIONS:	// - newIndexPage, newIndexPageId valid and pinned
												// - currentIndexPage, currentIndexPageId valid and pinned
			...
			for (tmpEntry = currentIndexPage.getFirst(delRid); tmpEntry != null; tmpEntry = currentIndexPage.getFirst(delRid)) 
			{
				...
				newIndexPage.insertKey(tmpEntry.key, ((IndexData) tmpEntry.data).getData());
							...
							// ASSERTIONS:// - currentIndexPage empty
		...
		else if (currentPage.getType() == NodeType.LEAF) 
		{
			...
			PageId currentLeafPageId = currentPageId;
										...
										// ASSERTIONS:// - currentLeafPage, currentLeafPageId valid and pinned
					...
					if (currentLeafPage.available_space() >= BT.getKeyDataLength(key, NodeType.LEAF)) 
			{
				...
				return null;
										...
										// ASSERTIONS:// - on the current leaf page is not enough space available.
											// It splits.
			...
			currentLeafPage.setNextPage(newLeafPageId);		// change the prevPage pointer on the next page:
			...
			// ASSERTIONS:// - newLeafPage, newLeafPageId valid and pinned// - currentLeafPage, currentLeafPageId valid and pinned
			...
			if (trace != null) 
			{
				...
				if (headerPage.get_rootId().pid != currentLeafPageId.pid)
					...
					trace.writeBytes("SPLIT node " + currentLeafPageId + " IN nodes " + currentLeafPageId+ " "+ newLeafPageId + lineSep);
					else
			...
			for (tmpEntry = currentLeafPage.getFirst(firstRid); tmpEntry != null; tmpEntry = currentLeafPage.getFirst(firstRid)) 
			{
				...
				newLeafPage.insertRecord(tmpEntry.key, ((LeafData) (tmpEntry.data)).getData());
							...
							// ASSERTIONS:// - currentLeafPage empty
...
RecordNotFoundException, PinPageException, IndexFullDeleteException, LeafDeleteException, IteratorException,ConstructPageException, DeleteRecException, IndexSearchException, IOException 
	{
...
/*
	...
	BTLeafPage findRunStart(KeyClass lo_key, RID startrid) throws IOException,IteratorException,KeyNotMatchException,ConstructPageException,PinPageException, UnpinPageException 
	{
		...
		while (curEntry == null) 
		{
			...
			curEntry = pageLeaf.getFirst(startrid);
											...
											// ASSERTIONS:// - curkey, curRid: contain the first record on the
...
/*
	...
	private boolean NaiveDelete(KeyClass key, RID rid) throws LeafDeleteException,KeyNotMatchException,PinPageException,ConstructPageException, IOException,
...
IteratorException, PinPageException,UnpinPageException, IteratorException
	...
	private KeyClass _Delete(KeyClass key, RID rid, PageId currentPageId, PageId parentPageId) throws IndexInsertRecException, RedistributeException, IndexSearchException, RecordNotFoundException,
...
IteratorException, PinPageException, IOException 
	{
		...
		if(trace!=null)
		{
			trace.writeBytes("current node " + currentPageId +lineSep);
			trace.flush();
		}
		
		if(currentPage.getType() == NodeType.INDEX)
		{
			System.out.println("going down to the leaf!");
			BTIndexPage currentIndexPage = new BTIndexPage(page,headerPage.get_keyType());
			PageId currentIndexPageId = currentPageId;
			PageId nextPageId;
			...
			upKey = _Delete(key,rid,nextPageId,currentPageId);
			
			System.out.println("going up to the root!");
			if(upKey == null)
				return null;				

			currentIndexPage= new  BTIndexPage(pinPage(currentPageId),headerPage.get_keyType() );
			
			RID currentRID = currentIndexPage.deleteKey(upKey);
			if(currentRID != null)
				System.out.println("Key is Deleted from index Page!");
			
			//case 1:Index Page is Root of the tree
			if(currentIndexPage.getCurPage().pid == headerPage.get_rootId().pid)
			{
				if(currentIndexPage.numberOfRecords() == 0)
				{
					System.out.println("It was root and now its empty!");
					updateHeader(currentIndexPage.getPrevPage());
					unpinPage(currentIndexPage.getCurPage());
					freePage(currentIndexPage.getCurPage());
					return null;
				}
				
			}
			
			//case 2:Index page is at least half full 
			if((currentIndexPage.available_space()+4) <= (currentIndexPage.MAX_SPACE-currentIndexPage.DPFIXED)/2)
			{
			...
			{
				System.out.println("We need to merge or redistribute!!!!!");
				BTIndexPage parent = new BTIndexPage(pinPage(parentPageId),headerPage.get_keyType());
				PageId siblingId = new PageId();
				int direction = parent.getSibling(upKey, siblingId);
				BTIndexPage sibling = new BTIndexPage(pinPage(siblingId),headerPage.get_keyType());
				
				//case 4:Merging or Redistribution is not possible
				if(direction == 0)
				{
					System.out.println("No sibling is there!Merging or redistribution isn't pssible!");
					unpinPage(currentIndexPage.getCurPage(),true);
					unpinPage(parent.getCurPage());
					unpinPage(sibling.getCurPage());
					return null;
				}
				
				//case 3:Redistribution or merging is possible
				else
				{
					//Merging
					if((currentIndexPage.available_space()+sibling.available_space()+8) > (currentIndexPage.MAX_SPACE-currentIndexPage.DPFIXED))
					{
						System.out.println("We are going to do merging!!");
						KeyClass moveUpKey = _Merge_Index(currentIndexPage,sibling,parent,currentRID,direction);
					...
					//Redistribution
					else if(sibling.redistribute(currentIndexPage, parent, direction, upKey))
					{   
						System.out.println("We are doing redistribution!");
						unpinPage(currentIndexPage.getCurPage(),true);
						unpinPage(parent.getCurPage());
						unpinPage(sibling.getCurPage());
						System.out.println("Redistribution is done!!");
						return null;
					}
					
					//Case 4: Merging and Redistribution not possible
					else
					{
						System.out.println("Merging or Redistribution isn't possible!");
						unpinPage(currentIndexPage.getCurPage(),true);
						unpinPage(parent.getCurPage());
						unpinPage(sibling.getCurPage());
						return null;
		...
		{
			System.out.println("We reached  the leaf!");
			BTLeafPage leaf = new BTLeafPage(page,headerPage.get_keyType());
			RID lRid = new RID();
			KeyDataEntry original = new KeyDataEntry(key,rid);
			KeyDataEntry temp = leaf.getFirst(lRid);
			
			while(temp!= null && BT.keyCompare(key, temp.key)>=0)
			{
				boolean b = leaf.delEntry(original);
				
				if(b)
				{
					System.out.println("Key is successfully deleted from leaf!!");
					
					//case 1:Leaf Page is Root of the tree
					if(leaf.getCurPage().pid == headerPage.get_rootId().pid)
					{
						System.out.println("Leaf is root!!");
						if(leaf.numberOfRecords() != 0)
						{
							System.out.println("But it isn't empty!!");
						...
						else
						{
							...
							unpinPage(leaf.getCurPage());
							freePage(leaf.getCurPage());
							updateHeader(new PageId(INVALID_PAGE));
							return null;
						}
					}
					
					//case 2:Leaf is at least half full
					else if((leaf.available_space()+4) <= (leaf.MAX_SPACE-leaf.DPFIXED)/2)
					{
						System.out.println("Everything is fine here!");
					...
					{
						System.out.println("We have to do merging or redistribution!");
						BTIndexPage parent = new BTIndexPage(pinPage(parentPageId),headerPage.get_keyType());
						PageId siblingId = new PageId();
						int direction = parent.getSibling(key, siblingId);
						BTLeafPage sibling = new BTLeafPage(pinPage(siblingId),headerPage.get_keyType());
						...
						//case 3: Merging or Redistribution is possible
						else
						{
							//Merging
							if((leaf.available_space()+sibling.available_space()+8) > (leaf.MAX_SPACE-leaf.DPFIXED))
							{
								System.out.println("We are going to do merging!!");
								KeyClass moveUpKey = _Merge_Leaf(leaf,sibling,rid,direction);
								System.out.println("Merging is done!!");
								unpinPage(parent.getCurPage());
								return moveUpKey;
							}
							...
							//Redistribution
							else if(sibling.redistribute(leaf, parent, direction, key))
							{
								System.out.println("We are doing redistribution!");
								unpinPage(leaf.getCurPage(),true);
								unpinPage(parent.getCurPage(),true);
								unpinPage(sibling.getCurPage(),true);
								System.out.println("Redistribution is done!!");
								return null;
							}
							
							//case4:Merging or Redistribution is not possible
							else
							{
								System.out.println("Merging or Redistribution isn't possible!");
								unpinPage(leaf.getCurPage(),true);
				...
				System.out.println("Key doesnt exist in this leaf go to the nexr leaf!");
				PageId nextLeafId = leaf.getNextPage();
				unpinPage(leaf.getCurPage());
				if(nextLeafId.pid == INVALID_PAGE)
				{
					System.out.println("NO SUCH KEY EXIST IN TREE!!!!");
				}
				leaf = new BTLeafPage(pinPage(nextLeafId),headerPage.get_keyType());
				temp = leaf.getFirst(rid);
			}
		}
		return null;
	}
	...
	public KeyClass _Merge_Index(BTIndexPage currentIndexPage,BTIndexPage sibling,BTIndexPage parent,RID currentRID,int direction) throws IteratorException,InsertRecException,DeleteRecException,IOException,FreePageException,IndexSearchException,IndexInsertRecException
	{
		KeyClass moveUpKey;
		KeyDataEntry temp;
		RID rid =new RID();
		
		System.out.println("We are in the _merge_index function!");
		
		if(direction == -1)
		{
			System.out.println("We are merging with left sibling!");
			moveUpKey = currentIndexPage.getFirst(currentRID).key;
			sibling.insertKey(parent.findKey(moveUpKey),sibling.getCurPage());
			
			for(temp = currentIndexPage.getFirst(rid);temp != null; temp = currentIndexPage.getFirst(rid))
			{
				sibling.insertRecord(temp);
				currentIndexPage.deleteSortedRecord(rid);
			}
			freePage(currentIndexPage.getCurPage());
			return moveUpKey;
		} 
		...
		{
			System.out.println("We are merging with right sibling!");
			moveUpKey = sibling.getFirst(currentRID).key;
			currentIndexPage.insertKey(parent.findKey(moveUpKey),currentIndexPage.getCurPage());
			
			for(temp = sibling.getFirst(rid);temp != null; temp = sibling.getFirst(rid))
			{
				currentIndexPage.insertRecord(temp);
				sibling.deleteSortedRecord(rid);
			}
			freePage(sibling.getCurPage());
			return moveUpKey;
		}
		
	}
	
	public KeyClass _Merge_Leaf(BTLeafPage leaf,BTLeafPage sibling,RID currentRID,int direction) throws IteratorException,InsertRecException,DeleteRecException,IOException,FreePageException,ConstructPageException,UnpinPageException
	{
		KeyClass moveUpKey;
		KeyDataEntry temp;
		RID rid =new RID();
		
		System.out.println("We are in the _merge_leaf function!");
		
		if(direction == -1)
		{
			System.out.println("We are merging with left sibling!");
			moveUpKey = leaf.getFirst(currentRID).key;
			//sibling.insertKey(parent.findKey(moveUpKey),currentIndexPage.getLeftLink());
			for(temp = leaf.getFirst(rid);temp != null; temp = leaf.getFirst(rid))
			{
				sibling.insertRecord(temp);
				leaf.deleteSortedRecord(rid);
			}
			sibling.setNextPage(leaf.getNextPage());
			if(leaf.getNextPage().pid != INVALID_PAGE)
			{
				BTLeafPage rightSibling = new BTLeafPage(leaf.getNextPage(),headerPage.get_keyType());
				rightSibling.setPrevPage(sibling.getCurPage());
				unpinPage(rightSibling.getCurPage(),true);
			}
			freePage(leaf.getCurPage());
			unpinPage(sibling.getCurPage());
			return moveUpKey;
		...
		else
		{
			...
			for(temp = sibling.getFirst(rid);temp != null; temp = sibling.getFirst(rid))
			{
				leaf.insertRecord(temp);
				sibling.deleteSortedRecord(rid);
			}
			
			leaf.setNextPage(sibling.getNextPage());
			if(sibling.getNextPage().pid != INVALID_PAGE)
			{
				BTLeafPage rightSiblingOfSibling = new BTLeafPage(sibling.getNextPage(),headerPage.get_keyType());
				rightSiblingOfSibling.setPrevPage(leaf.getCurPage());
				unpinPage(rightSiblingOfSibling.getCurPage(),true);
			}
			freePage(sibling.getCurPage());
			unpinPage(leaf.getCurPage(),true);
			return moveUpKey;
...
IteratorException, ConstructPageException, PinPageException, UnpinPageException
	...
	void trace_children(PageId id) throws IOException,IteratorException, ConstructPageException, PinPageException, UnpinPageException 
	{
